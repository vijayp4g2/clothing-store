    ...
    <div class="group">
        <div class="relative aspect-[3/4] overflow-hidden rounded-2xl bg-gray-200 dark:bg-gray-800 mb-4">
            ...
            <div class="absolute top-4 right-4 flex flex-col gap-2">
                <button onclick="toggleWishlist(${id}, '${name}', ${price.replace('₹', '').replace(',', '')}, '${image}')" class="h-10 w-10 flex items-center justify-center rounded-full bg-white/90 dark:bg-black/90 backdrop-blur-md text-gray-900 dark:text-white shadow-lg opacity-0 group-hover:opacity-100 transition-all hover:scale-110">
                    <span class="material-symbols-outlined text-[20px]">favorite</span>
                </button>
                <button onclick="quickView(event, '${name}', '${price}', '${image}')" class="h-10 w-10 flex items-center justify-center rounded-full bg-white/90 dark:bg-black/90 backdrop-blur-md text-gray-900 dark:text-white shadow-lg opacity-0 group-hover:opacity-100 transition-all hover:scale-110">
